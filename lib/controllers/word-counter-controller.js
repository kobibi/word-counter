...
router.get('/word-counter', function(req, res){
    try {
        const {query} = req;
        ...
        if (query.string) {
            ...
            res.status(422);
            res.send("Request must specify an input");
            return;
        }

        processString(query.string, inputType);
        res.json({success: true});
    }
    catch (err) {
        ...
        console.error(err);
        res.status(500);
        res.send("An error occurred while processing your request. Please try again later.")
    }
});
